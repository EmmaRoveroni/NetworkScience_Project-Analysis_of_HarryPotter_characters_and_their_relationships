...
for i in n_sample_to_remove:
    ...
    #mean centrality
    mean_bcen = np.mean(list(nx.closeness_centrality(new_graph).values()))
    mean_ccen = np.mean(list(nx.betweenness_centrality(new_graph).values()))
...
def homophily_removal(graph, nodes):
    graph_copy = graph.copy()
    nodes_to_remove = homophily_house_sorted[0:nodes]
    graph_copy.remove_nodes_from(nodes_to_remove)
    return graph_copy
